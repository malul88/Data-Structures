...
void Agency::treeToArray(TreeNode<int, int> *v, int *array, int *counter) {
    if (v == nullptr) {
        return;
    }
    treeToArray(v->left, array, counter);
...
void Agency::mergeArrays(int *arr1, int *arr2, int n1, int n2, int *arr3) {
    int i = 0, j = 0, k = 0;
    // Traverse both array
    while (i < n1 && j < n2) {
        // Check if current element of first
...
void Agency::arrayToTree(PriorityBySale *array, int m, RankTree<int, int> *t) {
    int h = log2((double) m);                      //heightNode of the tree
    int leafs_to_del = pow(2, h + 1) - m - 1;
    int current_car = 0;
...
void Agency::fillTreeWithInorder(TreeNode<int, int> *v, int m, int *current_car, PriorityBySale *array) {
    if (*current_car == m || v == nullptr) {
        return;
    }
    fillTreeWithInorder(v->left, m, current_car, array);
